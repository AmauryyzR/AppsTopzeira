...
export const LatexRenderer: React.FC<LatexRendererProps> = ({ content, className = '' }) => {
    ...
    while ((match = blockRegex.exec(content)) !== null) {
      ...
      parts.push(
        <div
          ...
          className="my-3 overflow-x-auto rounded-2xl bg-[#f5f5f7] p-4 text-center shadow-[0_1px_3px_rgba(0,0,0,0.03)]"
        >
  ...
  return <div className={`leading-relaxed text-neutral-800 ${className}`}>{elements}</div>;
};

// Helper to parse markdown bold (**text**) and italic (*text*)
function renderFormattedText(text: string, keyPrefix: string): React.ReactNode {
  ...
  while ((match = formatRegex.exec(text)) !== null) {
    ...
    if (plainText) {
      parts.push(<span key={`${keyPrefix}-p-${lastIndex}`}>{plainText}</span>);
    }
  ...
  if (remainder) {
    parts.push(<span key={`${keyPrefix}-p-end`}>{remainder}</span>);
  }
...
function renderInlineMath(text: string, keyPrefix: string): React.ReactNode {
  ...
  while ((match = inlineRegex.exec(text)) !== null) {
    ...
    if (rawText) {
      parts.push(renderFormattedText(rawText, `${keyPrefix}-fmt-${lastIndex}`));
    }
    const math = match[1];
    parts.push(
      <MathFormula
  ...
  const remainder = text.slice(lastIndex);
  if (remainder) {
    parts.push(renderFormattedText(remainder, `${keyPrefix}-fmt-end`));
  }
