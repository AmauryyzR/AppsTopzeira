...
export class SkinBuilder {
  ...
  constructor(private rig: StreetwearRig) {}
  ...
  finish(group: THREE.Group) {
    ...
    for (const [material, pieces] of this.buckets) {
      ...
      mesh.name = `Streetwear_${material.name}`;
    ...
    if (triangles >= 150_000) throw new Error(`Streetwear character exceeds triangle budget: ${triangles}`);
    group.userData.triangles = triangles;
    group.userData.boneCount = this.rig.bones.length;
    group.userData.rig = 'Humanoid A-pose, Y-up, +Z forward, articulated fingers';
  }
}
