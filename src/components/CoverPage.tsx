...
import { Calculator, Gamepad2, ArrowRight, BookOpen } from 'lucide-react';
...
export const CoverPage: React.FC<CoverPageProps> = ({ onOpenTab }) => {
  ...
  return (
    <div className="relative min-h-[calc(100vh-3.5rem)] w-full flex flex-col items-center justify-between px-4 py-8 md:py-14 overflow-y-auto bg-grid-pattern">
      ...
      <div className="w-full max-w-4xl z-10 px-2 sm:px-4 my-4">
        <div className="grid grid-cols-1 md:grid-cols-2 gap-6 md:gap-8">
          ...
          {/* Card 3: Topzeira Enem (Row 2, Col 1) */}
          <motion.div
            initial={{ opacity: 0, y: 30 }}
            animate={{ opacity: 1, y: 0 }}
          ...
          >
            <div className="relative w-full aspect-video rounded-3xl overflow-hidden glass-card p-6 border border-emerald-500/20 group-hover:border-emerald-400/80 transition-all duration-500 group-hover:shadow-[0_0_40px_rgba(16,185,129,0.35)] group-hover:-translate-y-2 flex flex-col justify-between">
              <div className="absolute inset-0 bg-gradient-to-br from-emerald-950/40 via-slate-950/80 to-teal-950/40 opacity-80 group-hover:opacity-100 transition-opacity duration-500" />
              <div className="relative z-10 my-auto pt-2">
      ...
      {/* Footer Info with hidden easter egg button placed further down */}
      <motion.div
        initial={{ opacity: 0, y: 10 }}
        animate={{ opacity: 1, y: 0 }}
        transition={{ delay: 0.7 }}
        className="mt-20 md:mt-32 mb-10 z-10 flex items-center justify-center"
      >
        <div
